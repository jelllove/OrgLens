...
[Download v0.2.2](https://github.com/jelllove/OrgLens/releases/tag/v0.2.2)
...
After building from source, use `artifacts\OrgLens-0.2.2\OrgLens.Preview.exe`.
...
Requirements: classic Outlook for Windows (32-bit or 64-bit), .NET Framework 4.8,
...
1. Download `OrgLens-0.2.2.zip` from [Releases](https://github.com/jelllove/OrgLens/releases/latest)
   and extract it to a local folder.
2. Close classic Outlook.
3. Run PowerShell in the extracted `OrgLens-0.2.2` folder:

   ```powershell
   powershell.exe -NoProfile -ExecutionPolicy Bypass -File .\Install.ps1
   ```

   `Bypass` applies only to this invocation, not to the machine's permanent policy.
   If your organization forbids it, ask IT to sign/approve the package instead.
   Administrator rights are not needed: registration is under the current user
   and the files are copied to `%LOCALAPPDATA%\OrgLens\Addin`.

4. Open classic Outlook. Select **OrgLens > Formatting rules**.
...
Close Outlook and run `Uninstall.ps1`. This removes only OrgLens's current-user
registration, not Outlook view settings or other add-ins. You can then delete
`%LOCALAPPDATA%\OrgLens\Addin`.
...
primary interop assemblies (PIAs) is required:
...
the sample UI smoke test without a separate test runner, and creates the ZIP package
and its `.zip.sha256` checksum under `artifacts`. The package includes the preview,
...
To verify a downloaded package in PowerShell, compare:

```powershell
(Get-FileHash .\OrgLens-0.2.2.zip -Algorithm SHA256).Hash.ToLowerInvariant()
Get-Content .\OrgLens-0.2.2.zip.sha256
```
